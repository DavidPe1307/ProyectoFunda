    ...
    <nav>
        <ul class="menu-horizontal">
            ...
            <li>
                ...
                <ul class="menu-vertical">
                    <li><a href="../frontend/interfaceslots_user.php">Visualizar slots</a></li>
                </ul>
            </li>
            <li>
...
function updateSlotsUI(slots) {
    ...
    slots.forEach(slot => {
        const el = document.querySelector(`.spot[data-id="${slot.id_slot}"]`);
        if (!el) return;
...
function fetchSlots() {
    fetch('../backend/fetch_slots.php')
        .then(response => response.json())
        .then(data => updateSlotsUI(data))
        .catch(error => console.error('Error cargando slots:', error));
}
